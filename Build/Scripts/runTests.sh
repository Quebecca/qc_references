...
shift $((OPTIND - 1))
if [ -n "${1}" ]; then
    TEST_FILE="Web/typo3conf/ext/qc_references/${1}"
else
    case ${TEST_SUITE} in
        functional)
            TEST_FILE="Web/typo3conf/ext/qc_references/Tests/Functional"
            ;;
        unit)
            TEST_FILE="Web/typo3conf/ext/qc_references/Tests/Unit"
            ;;
    esac
fi
